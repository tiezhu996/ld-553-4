...
@Component({ standalone: false,
  ...
  <h1>运营大屏</h1>
  <div class="grid grid-3" *ngIf="overview">
  ...
  <div class="grid grid-3" *ngIf="overview" style="margin-top: 12px;">
    <app-stats-card title="今日充电量" [value]="overview.today_charging_kwh + ' kWh'"></app-stats-card>
    <app-stats-card title="今日充电收入" [value]="'¥' + overview.today_charging_revenue"></app-stats-card>
    <app-stats-card title="今日充电次数" [value]="overview.today_charging_count"></app-stats-card>
  </div>
  <div class="grid charts">
    <section class="panel">
      <h2>订单趋势</h2>
      <div class="bars">
        <span *ngFor="let item of trend" [style.height.px]="20 + item.count * 8">{{ item.count }}</span>
      </div>
    </section>
    <section class="panel">
      <h2>收入统计</h2>
      <pre>{{ revenue | json }}</pre>
    </section>
    <section class="panel">
      <h2>车辆类型分布</h2>
      <pre>{{ revenue?.vehicle_distribution | json }}</pre>
    </section>
    <section class="panel">
      <h2>充电桩利用率</h2>
      <strong>{{ overview?.pile_utilization || 0 }}%</strong>
    </section>
  </div>
</section>`,
  styles: [
